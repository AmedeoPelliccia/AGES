...
| Functional engine | An autonomous functional unit with an explicit input/output contract, catalogued in [`engines/`](engines/README.md). GENIUSS, GENTILE and GTL are the proposed functional engines; they are mutually independent, none contains another, and conformance to AGES requires none of them. |
| Functional toolchain | A composition of functional engines connected only through contract artefacts ([`toolchains/`](toolchains/README.md)). A toolchain is defined by the engines' contracts, adds no capability its engines lack, carries no authority, and terminates at the governance boundary. Engine ≠ toolchain ≠ AGES. |
| Contract artefact | A semantically typed, inspectable, provenance-bound artefact through which functional engines compose without mutual dependencies ([`contracts/`](contracts/README.md)): for example SemanticContext, IntentArtefact and ActionCandidate. No contract artefact carries or confers authorisation. |
| GENIUSS | **GENIUSS — Graph Engine for Neural Integration and Understanding of Semantic Structures.** An integrative interpretation engine that converts heterogeneous observations, current context and prior semantic state into a contextualised semantic structure describing what the system currently understands about its environment, inputs, entities, relations and state. GENIUSS produces understanding, not intent, authorisation or action; it is a cognitive and semantic integration engine, not a graphical rendering engine, and implies no claim of machine consciousness or sentience. |
| Contextual semantic structure | The primary GENIUSS output: a structured, inspectable representation of entities, relations, roles, attributes, confidence, contextual hypotheses and temporal state, traceable to the observations from which it was derived. Distinct from a GENTILE semantic artefact, which represents negotiated intent. |
| Semantic hypothesis | A candidate interpretation retained with a declared confidence, its supporting and contradicting observations and the conditions under which it would be revised; competing hypotheses may coexist, and an inference must not be recorded as a fact. |
| Salience | A declared measure of the relevance attributed to an element of a contextual semantic structure for the current context; salience is an interpretive attribute, not a priority, an authority or an instruction. |
